...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    if GetResourceState('cf-inventory') ~= 'missing' then
        PlayerData.items = exports['cf-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid)
    end
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState('cf-inventory') ~= 'missing' then exports['cf-inventory']:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState('cf-inventory') ~= 'missing' then exports['cf-inventory']:SaveInventory(PlayerData, true) end
...
function QBCore.Player.SaveInventory(source)
    if GetResourceState('cf-inventory') == 'missing' then return end
    exports['cf-inventory']:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    if GetResourceState('cf-inventory') == 'missing' then return end
    exports['cf-inventory']:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    if GetResourceState('cf-inventory') == 'missing' then return end
    return exports['cf-inventory']:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    if GetResourceState('cf-inventory') == 'missing' then return end
    return exports['cf-inventory']:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    if GetResourceState('cf-inventory') == 'missing' then return end
    return exports['cf-inventory']:GetFirstSlotByItem(items, itemName)
end
